...
import com.aafes.starsettler.gateway.fdms.FirstDataFormatter;
 ...
 */
@Stateless
public class WexDataFormatter {
    
    private static final Logger logger = LoggerFactory.getLogger(WexDataFormatter.class.getName());
    @EJB
    ...
    public String formatRequestXML(List<SettleEntity> wexDataList) throws SAXException, IOException,
            XPathExpressionException, TransformerConfigurationException, TransformerException, ParserConfigurationException {

        logger.info("Entry in formatRequestXML method of WexDataFormatter..");

        if (wexDataList != null
                && wexDataList.size() > 0) {

            logger.info("formatRequestXML ## wexDataList is not empty");
            ...
            request.setPID(pid);
            request.setSID(sid);
            request.setBatchId(wexDataList.get(0).getBatchId());
            ...
            logger.info("Exit from formatRequestXML method of WexDataFormatter..");
            return marshal(request);
        } else {
            logger.info("formatRequestXML ## wexDataList is empty");
            return "";
        }
